...
export default function RecommendationResult({ recommendation, onBack = () => { }, onRestart = () => { } }) {
  ...
  const redirectToCheckout = (variantId, quantity, shopDomain) => {
    try {
      ...
      // Order Attribute (nivel pedido - Único método que funciona en Cart Permalinks)
      ...
      console.log('  🏷️ Order Attribute: _source=app_encuesta');
      console.log('  ℹ️  Flow debe buscar en: Order > customAttributes');
  ...
  const agregarAlCarrito = () => {
    try {
      ...
      // Construir la URL del carrito con order attribute
      const cartPermalink = `https://${shopDomain}/cart/${items.join(',')}`;
      
      const params = new URLSearchParams();
      // Order Attribute (nivel pedido - funciona en Cart Permalinks)
      params.append('attributes[_source]', 'app_encuesta');
      ...
      console.log('  🏷️ Order Attribute: _source=app_encuesta');
      console.log('  ℹ️  Flow debe buscar en: Order > customAttributes (NO en lineItems)');
